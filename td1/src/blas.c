	   ...
	   double *C, const int ldc)
{
  ...
  if(TransA != CblasNoTrans){
	...
	init_matrice(tmp,t_m,t_n,t_m,0.0);
	for (int k =0; k < nb_k; k++){
	  cblas_dgemm_scalaire_gen(t_m, t_n, t_k, A+i*lda*t_m+k*t_k, t_m, B+j*ldb*t_n+k*t_k, t_n, tmp, t_m);
	  somme_matrice(t_m,t_n,C+j*ldc*t_n+i*t_m, tmp, C+j*ldc*t_n+i*t_m)
	}
      }
    }
  }
